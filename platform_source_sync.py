...
def main() -> int:
    ...
    while True:
        ...
        print(
            "source sync cycle sources=%s processed=%s fetched=%s normalized=%s candidates=%s created_jobs=%s existing_jobs=%s skipped=%s failed=%s retry_checked=%s retry_created=%s route_retry_checked=%s route_retry_created=%s"
            % (
                ...
                int(summary.get("route_retry_checked_count") or 0),
                int(summary.get("route_retry_created_job_count") or 0),
            ),
            flush=True,
        )
        if config.source_sync.once:
