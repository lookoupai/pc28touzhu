...
- 新轮实际准备派单时重置路由基线及倍投步数；活动轮跨天继续时不重置。定时规则按启动日限制每天最多新开一次：旧轮凌晨结束不占用当天次数；窗口内旧轮仍运行时等待，结束后可在尚未结束的主窗口或备用窗口开新轮。
- 旧版跨日 `blocked` 占位不代表已经启动。只有通过窗口、信号和运行状态检查后，才会原子复用没有关联投注事件或运行轮次的空占位；派单失败可重试，历史账目保留。
...
| 跟单状态 | 信号采集 | 普通直派 | 自动规则 |
| --- | --- | --- | --- |
| `active` | 来源启用时采集 | 按策略和群组派单模式执行 | 可触发 |
| `standby` | 来源启用时采集 | 不执行 | 可触发，已启动路由可连续派单 |
| `inactive` | 该订阅不要求采集 | 不执行 | 不触发 |
| `archived` | 该订阅不要求采集 | 不执行 | 不触发 |

仅由自动路由执行的方案使用 `standby`。条件或定时路由开轮不再把它改成 `active`，因此停止规则后不会留下普通直派。无路由的旧规则保留原有激活订阅行为。

规则编辑器允许选择未归档的停用方案，并显示其状态；选择或启用规则不会自动解除来源或跟单方案的停用。来源必须为 `active`、跟单方案必须为 `active` 或 `standby` 才能执行，页面会列出未就绪依赖。
